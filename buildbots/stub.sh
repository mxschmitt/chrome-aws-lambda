...
echo "Running Playwright lambda function"
docker run --rm -v "$PWD":/var/task:ro,delegated lambci/lambda:nodejs12.x buildbots/playwright.handler '{"url": "https://example.com"}' &> "$LOG_FILE"
grep -Fq "Example Domain" "$LOG_FILE"
echo "Sucessfully passed Playwright test"
